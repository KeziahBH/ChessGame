...
const GameInfo = ({ streak }) => {
  ...
  // const BASE_URL = 'http://api-gateway-service:8080' ;
  ...
  const cancelSearch = async () => {
    ...
    try {
      await fetch(`${BASE_URL}/api/v1/match/cancel-waiting`, {
        method: 'POST',
        credentials: 'include',
        headers: {
  ...
  const pollForMatch = (type) => {
    ...
    pollingIntervalRef.current = setInterval(async () => {
      ...
      try {
        const response = await fetch(`${BASE_URL}/api/v1/match/check-match`, {
          method: 'GET',
          credentials: 'include',
        });
  ...
  const createNewGame = async (type = "classical") => {
    ...
    try {
      ...
      // endpoint not found
      const response = await fetch(`${BASE_URL}/api/v1/match`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
    ...
    <div className="game-info">
      ...
      <div className="game-type-selector">
        ...
        <div className="game-type-buttons">
          ...
          <button 
            className={`game-type-btn ${gameType === 'blitz' ? 'active' : ''}`}
            onClick={() => !isSearching && setGameType('blitz')}
            disabled={isSearching}
          >
            <FaBoltLightning size={20} />
            <div className="game-type-info">
              <span className="game-type-name">Blitz</span>
              <span className="game-type-time">3 min each</span>
            </div>
          </button>
        </div>
      </div>
      ...
      <div className="play-button-container">
        ...
        >
          {isSearching ? (
            ...
            <>
              {gameType === 'rapid' ? <FaClock size={20} /> : gameType === 'blitz' ? <FaBoltLightning size={20} /> : <FaChessPawn size={20} />}
              Play {gameType === 'rapid' ? 'Rapid (10 min)' : gameType === 'blitz' ? 'Blitz (3 Min)' : 'Classical'}
            </>
          )}
        </button>
      </div>
